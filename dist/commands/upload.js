...
const error = (str) => console.log('Error: ' + str);
const sanitize = (f, outDir) => f.replace(/\\+/g, '/').replace(outDir + '/', '');
;
export async function upload(filesAttr, opts) {
    if (!account?.email)
        return error(`Not logged in. Run 'micrio login' first`);
    if (!hasbin.sync('vips'))
        return error('Libvips not installed. Download it from https://www.libvips.org/install.html');
    let url;
    try {
    ...
    for (let i = 0; i < files.length; i++)
        try {
            ...
            if (queue.length >= PROCESSING_THREADS)
...
const walkSync = (dir, callback) => fs.lstatSync(dir).isDirectory()
    ...
    : callback(dir);
async function handle(f, outDir, folder, format, type, idx, total, omniId, setOmniId) {
    if (!fs.existsSync(f))
    ...
    if (!res)
        throw new Error('Could not create image in Micrio! Do you have the correct permissions?');
    ...
    if (isOmni) {
        if (!omniId)
            setOmniId(outDir + '/' + res.id);
        fs.mkdirSync(outDir + '/' + res.id);
        fs.renameSync(outDir + '/' + res.id + '_files', outDir + '/' + res.id + '/' + idx);
    }
    else
        fs.renameSync(outDir + '/' + res.id + '_files', outDir + '/' + res.id);
    const [height, width] = (/Height\="(\d+)"\n.*Width\="(\d+)"/m.exec(fs.readFileSync(outDir + '/' + res.id + '.dzi', 'utf-8')) ?? [0, 0, 0])
        .slice(1).map(Number);
    if (!height || !width)
    ...
    async function getUploadUris() {
        ...
        if (files.length)
            uploadUris.push(...await api(`/api/${folder.split('/')[1]}/store`, { files: files.map(f => sanitize(f, outDir)) }).then(r => {
                if (!r)
                    throw new Error('Upload permission denied.');
    ...
    while (allTiles.length) {
        ...
        if (queue.length >= UPLOAD_THREADS)
            await Promise.any(queue);
        if (!uploadUris.length)
            await getUploadUris();
        const tile = allTiles.shift();
        if (!tile)
            throw new Error('Could not get tile to upload.');
    ...
    // Finalize
    if (!omniId)
        await api(`/api/cli${folder}/@${res.id}?w=${width}&h=${height}&f=${format}&l=${total}`);
    // Move tile for base.bin generation
    if (isOmni) {
        if (!omniId && !fs.existsSync('basebin'))
...
function log(str, pos, newLine = false) {
    if (!newLine)
        newLine = pos == undefined;
    if (!newLine) {
